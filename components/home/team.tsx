        ...
        >
          {members.map((member, index) => (
            <div
              ...
              aria-label={`Team member: ${member.name}, ${member.role}`}
            >
              {/* Hover fill - removed since dark bg is default */}
              ...
              <div className="relative z-10 flex items-center gap-4">
                ...
                >
                  {member.image ? (
                    ...
                    <Users className="w-6 h-6 text-white transition-colors duration-300" />
                  )}
                  {/* Online dot */}
                  <span className="absolute bottom-0.5 right-0.5 w-3 h-3 bg-green-500 rounded-full border-2 border-white" />
                </div>
              ...
              {member.socials && (
                <div className="relative z-10 flex items-center gap-2 mt-auto pt-1">
                  {member.socials.linkedin && (
                    ...
                    >
                      <Link2Icon className="w-3.5 h-3.5 text-[#c9a84c]" />
                    </a>
                  )}
                  {member.socials.email && (
                    <a href={`mailto:${member.socials.email}`}
